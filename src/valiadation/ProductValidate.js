...
const  ProductValidate = (name,category,price ) => {
  ...
  else if (!isCategoryValid) return "Category is not valid";
  if(!price.trim()) return "Price is required";
  else if(!isPriceValid) return "Price is not valid";
  else if(parseFloat(price) <= 0) return "Price must be a positive number";
